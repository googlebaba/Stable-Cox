...
duration_col='Reccur.months'
event_col='Reccur.status'
def get_args():
...
def main(args, round, logger):
    ...
    training_data.drop(['Survival.status', 'Survival.months'], axis=1, inplace=True)
    test0_data.drop(['Survival.status', 'Survival.months'], axis=1, inplace=True)
    test1_data.drop(['Survival.status', 'Survival.months'], axis=1, inplace=True)
    test2_data.drop(['Survival.status', 'Survival.months'], axis=1, inplace=True)
    test3_data.drop(['Survival.status', 'Survival.months'], axis=1, inplace=True)
    
    test4_data.drop(['Survival.status', 'Survival.months'], axis=1, inplace=True)
    test5_data.drop(['Survival.status', 'Survival.months'], axis=1, inplace=True)
    test6_data.drop(['Survival.status', 'Survival.months'], axis=1, inplace=True) 
    test7_data.drop(['Survival.status', 'Survival.months'],  axis=1, inplace=True)
    ...
    if args.reweighting == "DWR":
        ...
        W = SRDO(X_train_np, p_s, hidden_layer_sizes = (38, 9), decorrelation_type="global", max_iter=args.iters_balance, random_state=args.seed)
    else:
        W = np.ones((n, 1))/n
    ...
    if args.paradigm == "regr":
        ...
        model = model_func(X_train_pd, duration_col, event_col, W, 0.0004, **vars(args))
