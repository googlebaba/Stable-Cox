...
def main(args, round, logger):
    ...
    training_pd_data = pd.read_csv('./omics_data/mela_cancer/train_median.csv', index_col=0)
    test0_pd_data = pd.read_csv('./omics_data/mela_cancer/test0_median.csv', index_col=0)
    test1_pd_data = pd.read_csv('./omics_data/mela_cancer/test1_median.csv', index_col=0)
    test2_pd_data = pd.read_csv('./omics_data/mela_cancer/test2_median.csv', index_col=0)
    test3_pd_data = pd.read_csv('./omics_data/mela_cancer/test3_median.csv', index_col=0)

    training_pd_data['Survival.months'] = training_pd_data['Survival.months']/30.0    
    test0_pd_data['Survival.months'] = test0_pd_data['Survival.months']/30.0
    test1_pd_data['Survival.months'] = test1_pd_data['Survival.months']/30.0

    test2_pd_data['Survival.months'] = test2_pd_data['Survival.months']/30.0
    test3_pd_data['Survival.months'] = test3_pd_data['Survival.months']/30.0



    tmp_data = training_pd_data.columns[2:]
    # 选择方差最大的三列
    top_3_cols = tmp_data
    ...
    if args.reweighting == "DWR":
        ...
        W = SRDO(X_train_np, p_s, hidden_layer_sizes = (30, 9), decorrelation_type=args.decorrelation_type, max_iter=args.iters_balance, random_state=args.seed)
    else:
        W = np.ones((n, 1))/n
    ...
    if args.paradigm == "regr":
        ...
        model = model_func(X_train_pd, duration_col, event_col, W, 0.0003, **vars(args))
    ...
    c_index_dict = []
    print("test0:")
    c_index0 = plot_KM(cph2, sorted_indices, coef, X_test0_pd)
    print(c_index0)
    ...
    print("test2")
    c_index2 = plot_KM(cph2, sorted_indices, coef, X_test3_pd)
    print(c_index2)
