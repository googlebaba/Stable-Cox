...
duration_col = 'Recurr.months'
event_col = 'Recurr.status'
def get_args():
...
def main(args, round, logger):
    ...
    test2_pd_data = test2_pd_data.drop(['Survival.months', 'Survival.status', 'Cohort'], axis=1)
    ...
    if args.reweighting == "DWR":
        ...
        W = SRDO(X_train_np, p_s, hidden_layer_sizes = (15, 13), decorrelation_type=args.decorrelation_type, max_iter=args.iters_balance, random_state=args.seed)
    else:
        W = np.ones((n, 1))/n
